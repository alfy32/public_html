...
<?php 
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$appointment[$row['schedule_col']][intval($time[0])][intval($time[1])]['display'] = 'main';
		
		$time_end = explode(':', $row['end_time']);
		
		$time[0] = intval($time[0]);
		$time[1] = intval($time[1]);
		$time_end[0] = intval($time_end[0]);
		$time_end[1] = intval($time_end[1]);
		...
		while($time[0] <= $time_end[0] && $time[1] < $time_end[1] || 
				$time[0] < $time_end[0])
		{
			$appointment[$row['schedule_col']][intval($time[0])][intval($time[1])]['appointment_id'] = $row['appointment_id'];
			$appointment[$row['schedule_col']][intval($time[0])][intval($time[1])]['client'] = $row['client'];
			...
			$appointment[$row['schedule_col']][intval($time[0])][intval($time[1])]['start_time'] = $row['start_time'];
			$appointment[$row['schedule_col']][intval($time[0])][intval($time[1])]['end_time'] = $row['end_time'];
			$appointment[$row['schedule_col']][intval($time[0])][intval($time[1])]['display'] = 'secondary';
	...
	function get_cell($col, $time, $pos)
	{
		...
		if($appointment[$col][$hour][$min]['appointment_id']) 
		{
			...
			$display = $appointment[$col][$hour][$min]['display'];
			
			if($display == 'secondary')
			{
				return "
					<td class='cell_{$min}' >
						<p class='write_{$pos}' onclick='edit_appointment(\"{$appointment_id}\",\"{$client}\",\"{$employee_id}\",\"{$date}\",\"{$start_time}\",\"{$end_time}\")'>x</p>
